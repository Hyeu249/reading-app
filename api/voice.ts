  ...
  constructor({ apiKey = "" }: { apiKey?: string }) {
    ...
    this.models = {
      generateContentStream: async function* ({
        ...
        // Mỗi chunk mô phỏng 1 phần audio
        for (let i = 0; i < contents.length; i++) {
          const textPart = contents[i].parts[0].text;

          // Fake chunk trả về inlineData base64
          const fakeAudioData = Buffer.from(`FAKE_AUDIO_DATA_${i}`).toString(
            "base64"
          );
          yield {
            candidates: [
